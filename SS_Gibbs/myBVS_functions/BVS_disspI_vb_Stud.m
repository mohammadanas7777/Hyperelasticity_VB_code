function out_vb = BVS_disspI_vb_Stud(X, y, initz0, tol, verbosity)
% This function implements VB for a Student's-t slab with fixed p0
...
if(isempty(initz0))
    ...
    DS = run_VB_Stud(X, y, A, B, av, bv, A0z, p0, initz, tol, verbosity);
end
...
function [DS, LLcvg] = run_VB_Stud(Xc, yc, A, B, av, bv, A0z, p0, initz, tol, verbosity)
...
termstore = 0.5*p - 0.5*N*log(2*pi) + A*log(B) - gammaln(A)...
            + av*log(bv) - gammaln(av) - 0.5*log(detA0z);
...
%     lambda = psi(apbar) - psi(bpbar); 
    ...
    LL(iter) = termstore - Abar*log(Bbar) + gammaln(Abar)...
        - avbar*log(bvbar) + gammaln(avbar) + 0.5*(tausig*tauv)*term5...
        + 0.5*log(det(Sigma)) + nansum( zm.*(log(p0) - log(zm)) )... 
