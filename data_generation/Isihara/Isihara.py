...
for j in range(0,len(loadsteps)):
    ...
    isExist = os.path.exists("../../FEM_data/Isihara/"+str(loadsteps[j]))
    
    if not isExist:
    # Create a new directory because it does not exist
       os.makedirs("../../FEM_data/Isihara/"+str(loadsteps[j]))
    ...
    df.to_csv("../../FEM_data/Isihara/"+str(loadsteps[j])+"/connectivity.csv",
    ...
    W = 0.5*(I1_bar -3) + 1.0*(I2_bar - 3) + 1.0*(I1_bar - 3)**2 + 1.5*((J-1)**2)
    ...
    df.to_csv("../../FEM_data/Isihara/"+str(loadsteps[j])+"/coordinates_and_disp.csv",
    ...
    df.to_csv("../../FEM_data/Isihara/"+str(loadsteps[j])+"/reactions.csv",
    ...
    #Define function
    Wa = 0.5*(i1bar -3) + 1.0*(i2bar - 3) + 1.0*(i1bar - 3)**2 + 1.5*((ja-1)**2)
