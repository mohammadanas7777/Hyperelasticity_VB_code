...
% subplot(2,3,[1,2,3])
stem(theta_truth,LineWidth=2)
hold on
stem(theta_DISSPi,'--*',LineWidth=2)
hold on 
stem(theta_VB,'--*',LineWidth=2)
xlabel("feature number",FontSize=25)
set(gca,'XTick',(0:1:NF),'FontSize',18)
ylabel("corresponding coefficient,\theta_i",FontSize=25)
title('Feature parameter value vs Feature number',FontSize=35)
legend({'\theta_{truth}','\theta_{MCMC}','\theta_{VB}'},'Location','northwest','FontSize',18)
...
% set(gca,'FontSize',18)
% ylabel("strain energy, W(\lambda)",FontSize=25)
% title('Biaxial tension',Fontsize=35)
% legend({'W_{truth}','W_{MCMC}','W_{VB}'},'Location','northwest','FontSize',18)
% lambda = linspace(0,1,100);
% W_truth = zeros(NF,1);
% W_MCMC = zeros(NF,1);
% for i =1:100
...
% set(gca,'FontSize',18)
% ylabel("strain energy, W(\lambda)",FontSize=25)
% title('Simple shear',Fontsize=35)
% legend({'W_{truth}','W_{MCMC}','W_{VB}'},'Location','northwest','FontSize',18)
% lambda = linspace(0,1,100);
% W_truth = zeros(NF,1);
% W_MCMC = zeros(NF,1);
