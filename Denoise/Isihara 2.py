...
alpha = np.arange(1e-9,1,0.1)
gamma = np.arange(1e-6,0.01,10)
...
for m in range(0,len(loadsteps)):  
    ...
    print('.')
    print('.')
    df = pd.read_csv("../FEM_data/Isihara_2/"+str(loadsteps[m])+"/coordinates_and_disp.csv") 
    ...
    a = 0.001*np.ones(int(NN/2))+(noise_high * np.array(torch.randn(int(NN/2))))
    b = -0.001*np.ones(int(NN/2))+(noise_high * np.array(torch.randn(int(NN/2))))
    u_x_noise_high = np.concatenate((a, b), axis = 0)
    np.random.shuffle(u_x_noise_high)
    ...
    df2.to_csv("../FEM_data/Isihara_2/"+str(loadsteps[m])+"/coordinates_and_disp.csv",
